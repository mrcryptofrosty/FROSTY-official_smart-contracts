...
const FrostyGenesisNFT = artifacts.require('FrostyGenesisNFT');
const FrostyGenesisNFTMock = artifacts.require('FrostyGenesisNFTMock');
const AlwaysRevertingEthReceiver = artifacts.require('AlwaysRevertingEthReceiver');

contract('Core NFT tests for FrostyGenesisNFT', function ([admin, multisig, buyer, buyer2, owner, smart_contract, ...otherAccounts]) {
    ...
    beforeEach(async () => {
        ...
        this.token = await FrostyGenesisNFTMock.new(
        ...
        it('Reverts when trying to transfer before the end of the genesis', async () => {
            ...
            await expectRevert(
              this.token.transferFrom(buyer, admin, TOKEN_ONE_ID, {from: buyer}),
                "FrostyGenesisNFT._beforeTokenTransfer: Transfers are currently locked at this time"
            );
        });
    });
    ...
    describe('buy()', () => {
       it('Successfully buys a genesis NFT', async () => {
        ...
        it('cannot buy() more than max total genesis contribution NFTs', async () => {
          ...
          await expectRevert(
            ...
            "FrostyGenesisNFT.buy: Total number of genesis token holders reached"
          );
        });
       ...
       it('Reverts when transfer to multisig fails', async () => {
           ...
           this.tokenWithRevertingMultisig = await FrostyGenesisNFTMock.new(
               this.accessControls.address,
           ...
           await expectRevert(
                ...
                "FrostyGenesisNFT.buy: Unable to send contribution to funds multisig"
           );
       });
    ...
    describe('adminBuy()', () => {
       ...
       it('Reverts when beneficiary is address zero', async () => {
           await expectRevert(
               ...
               "FrostyGenesisNFT.adminBuy: Beneficiary cannot be ZERO"
           );
       });
      ...
      it('Reverts when beneficiary already owns a genesis NFT', async () => {
        ...
        await expectRevert(
          ...
          "FrostyGenesisNFT.adminBuy: Beneficiary already owns a genesis NFT"
        );
      });
    });
    ...
    describe('updateGenesisEnd()', () => {
        ...
        it('Reverts when being changed for a second time', async () => {
          await this.token.updateGenesisEnd('900', {from: admin});
           await expectRevert(
               ...
               "FrostyGenesisNFT.updateGenesisEnd: End time locked"
           )
        });
        ...
        it('Reverts when end time already passed and trying to reopen', async () => {
          ...
          await expectRevert(
               this.token.updateGenesisEnd('99', {from: admin}),
               "FrostyGenesisNFT.updateGenesisEnd: End time already passed"
           )
        });
    });
    ...
    describe('updateAccessControls()', () => {
        ...
        it('Reverts when updating to address zero', async () => {
           await expectRevert(
               ...
               "FrostyGenesisNFT.updateAccessControls: Zero Address"
           );
        });
    })
});
